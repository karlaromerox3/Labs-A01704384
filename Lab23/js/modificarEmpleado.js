...
$escolaridad.keyup(function(){
    //si el campo no está vacio verificarlo.
    if($escolaridad.val() === null | $nombreEmpleado.val() === "")
    {
        $escolaridad.prop("required", false);
        $escolaridad.removeClass("is-invalid");
        $escolaridad.removeClass("is-valid");

    }
    else
    {
        $escolaridad.prop("required", true);
        validarNombres($escolaridad);
    }
});
...
function modDatosPersonal(id){
    ...
    if(!validarRFC($rfc) | !validarCURP($curp))
        valido = false;
...
function modDatosContacto(id, dirC){
    let valido = true;
    //Si no pasa alguna validación obligatoria dejar de ejecutar la función
    if(dirC == 1){
        //si la direccion no está completa
...
function modDatosContratacion(id){
    let valido = true;
    //Si no pasa alguna validación obligatoria dejar de ejecutar la función
...
function modDatosNomina(id){
    let valido = true;

    if(!validarSelect($idEstCivil))
        valido = false;
    //si el campo curp no está vacio verificarlo.
    if(!($escolaridad.val() === null | $escolaridad.val() === ""))
    {
        $escolaridad.prop("required", true);

        if(!validarNombres($escolaridad))
            valido = false;
    }
